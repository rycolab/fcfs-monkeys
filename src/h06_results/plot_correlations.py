...
def plot_frequency_length(df, results_path):
    df = df.copy()
    ...
    df = pd.melt(df, id_vars=['Language', 'seed'], var_name='Text', value_name='Correlation',
                 ...
                 # value_vars=['Natural', 'FCFS', 'PolyFCFS', 'Caplan', 'PolyCaplan'])
                 # value_vars=['Natural', 'Caplan', 'FCFS', 'PolyCaplan', 'PolyFCFS'])
...
def plot_polysemy_length(df, results_path):
    df = df.copy()
...
def plot_freq_length(checkpoints_path, results_path):
    df = read_results(checkpoints_path)

    plot_frequency_length(df, results_path)
    plot_polysemy_length(df, results_path)


def main():
    args = get_args()

    plot_freq_length(args.checkpoints_path, args.results_path)
